...
#define SERVOFAKTORLEFT 730
#define SERVOFAKTORRIGHT 590

// Zero-position of left and right servo
// When in calibration mode, adjust the NULL-values so that the servo arms are at all times parallel
// either to the X or Y axis
#define SERVOLEFTNULL 2300
#define SERVORIGHTNULL 690
...
{ 
  ...
  // Set current time only the first to values, hh,mm are needed
  setTime(timeClient.getCurrentEpochWithUtcOffset());
//  setTime(timeClient.getHours().toInt(),timeClient.getMinutes().toInt(),timeClient.getSeconds().toInt(),
//    timeClient.getDay().toInt(),timeClient.getMonth().toInt(),timeClient.getYear().toInt());
#endif
